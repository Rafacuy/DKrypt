...
from core.validation.validators import Validator, ValidationError
...
class CommandValidator:
    ...
    def validate_module_options(self, module: str, options: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for opt_name, opt_info in module_opts.items():
            if opt_info.get('required', False):
                if opt_name.upper() not in [k.upper() for k in options.keys()]:
                    errors.append(f"Required option '{opt_name}' is missing")
    ...
    def validate_and_get_suggestions(self, module: str, options: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
        ...
        if not is_valid:
            ...
            for error in validation_errors:
                ...
                if "Required option" in error and "is missing" in error:
                    try:
                        opt_name = error.split("'")[1]
                        opt_info = module_opts.get(opt_name, {})
                        description = opt_info.get('description', 'No description')
                        suggestions.append(f"Set {opt_name}: {description}")
