...
class VulnStatus(Enum):
    ...
    ERROR = "🔥"
    TIMEOUT = "⏰"
    BLOCKED = "🧱" # New status for WAF/Blocking
...
class ConfidenceLevel(Enum):
    ...
    MEDIUM = "Medium"
    LOW = "Low"
    INFO = "Info" # New level for informational findings
...
class BaselineComparator:
    ...
    def __init__(self):
        ...
        # Add WAF/blocking patterns
        self.waf_patterns = [
            "blocked", "forbidden", "not acceptable", "not authorized",
            "access denied", "security policy", "firewall", "waf"
        ]
    ...
    def _is_waf_response(self, response: Dict[str, Any]) -> bool:
        """
        Check if a response indicates WAF/firewall blocking.
        """
        ...
        # Check response body for WAF-related keywords
    ...
    def compare_response(self, test_response: Dict[str, Any]) -> Tuple[ConfidenceLevel, str]:
        ...
        # check for explicit server rejections or WAF blocks
        ...
        if self._is_waf_response(test_response):
            return ConfidenceLevel.INFO, f"Request appears to be blocked by a WAF/security policy (Status: {test_status})."
        ...
        # OPTIMIZATION: Use standard deviation for more robust timing analysis
        time_std_dev = statistics.stdev(baseline_times) if len(baseline_times) > 1 else 0
        ...
        # 1. Timing Analysis (Refined)
        test_time = test_response.get('elapsed', 0)
        # A significant delay is now defined as being more than 3 standard deviations from the mean
        if time_std_dev > 0 and (test_time > avg_baseline_time + 3 * time_std_dev):
            confidence_factors.append(0.8)
            details.append(f"Significant delay: {test_time:.2f}s vs avg {avg_baseline_time:.2f}s (StdDev: {time_std_dev:.2f}s)")
        ...
        if avg_baseline_length > 0 and abs(test_content_length - avg_baseline_length) > avg_baseline_length * 0.5:
...
class RequestSmuggler:
    ...
    def test_payload(self, payload_data: Dict[str, Any], progress: Progress, task_id: Any):
        ...
        # OPTIMIZATION: More specific exception handling
    ...
    def _save_txt_file(self, filename):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
                ...
                for i, result in enumerate(self.results, 1):
                    ...
                    status_clean = status_clean.replace('[blue]', '').replace('[/blue]', '')
                    ...
                    conf_clean = conf_clean.replace('[blue]', '').replace('[/blue]', '')
